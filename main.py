...
def extract_user_from_jwt(auth_header: Optional[str]) -> Tuple[Optional[str], str]:
    """Extract user_id and email from Supabase JWT. Returns (user_id, email)."""
    ...
    try:
        ...
        return payload.get("sub"), payload.get("email", "anonymous")
    except Exception as exc:
        print(f"⚠️ JWT extraction failed: {exc}")
        return None, "anonymous"
...
async def save_workflow_analysis(
    ...
    top_5_projects: List[Dict[str, Any]],
) -> None:
    """Save workflow analysis to Supabase (fire-and-forget)."""
    if not SUPABASE_URL or not SUPABASE_KEY:
        print("⚠️ Supabase not configured, skipping workflow save")
        return

    projects_to_save = []
    for project in top_5_projects[:5]:
        ...
        projects_to_save.append({
            ...
            "investment_rating": props.get("investment_rating"),
            "coordinates": project.get("geometry", {}).get("coordinates"),
            "component_scores": props.get("component_scores", {}),
        })
    ...
    payload = {
        ...
        "top_5_projects": projects_to_save,
    }
