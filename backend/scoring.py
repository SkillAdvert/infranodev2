"""
Scoring algorithms module for infrastructure investment analysis.

This module provides persona-based scoring, component scoring functions,
and investment rating calculations. Imports from proximity module for spatial functions.
"""
...
# Display and rating functions
def get_color_from_score(score_out_of_100: float) -> str:
    """
    Convert a score (0-100) to a color code.

    Args:
        score_out_of_100: Score from 0 to 100
...
def get_rating_description(score_out_of_100: float) -> str:
    """
    Convert a score (0-100) to a descriptive rating.
...
def calculate_capacity_component_score(
    ...
    """
    Calculate capacity score based on persona preferences.

    Args:
        capacity_mw: Capacity in megawatts
        persona: Data center persona type
        user_ideal_mw: User-specified ideal capacity (overrides persona defaults)
...
def calculate_development_stage_score(status: str, perspective: str = "demand") -> float:
    """
    Calculate development stage score based on project status.

    Args:
        status: Development status string
        perspective: Scoring perspective ("demand" or "supply")
...
def calculate_technology_score(tech_type: str) -> float:
    """
    Calculate technology score based on technology type.

    Args:
        tech_type: Technology type string
...
def calculate_grid_infrastructure_score(proximity_scores: Dict[str, float]) -> float:
    """
    Calculate grid infrastructure score based on proximity to substations and transmission.

    Args:
        proximity_scores: Dictionary containing nearest_distances
...
def calculate_digital_infrastructure_score(proximity_scores: Dict[str, float]) -> float:
    """
    Calculate digital infrastructure score based on proximity to fiber and IXPs.
...
def calculate_water_resources_score(proximity_scores: Dict[str, float]) -> float:
    """
    Calculate water resources score based on proximity to water sources.
...
def calculate_lcoe_score(development_status_short: str) -> float:
    """
    Calculate LCOE score based on development status.

    Args:
        development_status_short: Short development status string
...
def calculate_tnuos_score(project_lat: float, project_lng: float) -> float:
    """
    Calculate TNUoS (Transmission Network Use of System) score based on location.
    ...
    Returns:
        Score from 0 to 100 (higher is better, representing lower tariffs)
    """
...
def estimate_capacity_factor(
    ...
    """
    Estimate capacity factor based on technology type and location.

    Args:
        tech_type: Technology type string
        latitude: Project latitude
        user_provided: User-provided capacity factor (overrides estimation)

    Returns:
        Capacity factor as a percentage (0-100)
    """
...
def calculate_resilience_score(
    ...
    """
    Calculate resilience score based on infrastructure redundancy and backup capabilities.
...
def calculate_price_sensitivity_score(
    ...
    """
    Calculate price sensitivity score based on estimated costs.

    Args:
        project: Project dictionary
        proximity_scores: Dictionary containing nearest_distances
        user_max_price_mwh: User-specified maximum price per MWh
...
# Persona scoring helper functions
def _build_shared_persona_component_scores(
    ...
    """Build component scores that are shared across all personas."""
...
def build_persona_component_scores(
    ...
    """
    Build complete set of component scores for a project.

    Args:
        project: Project dictionary
        proximity_scores: Dictionary containing infrastructure proximity data
        persona: Data center persona type
        perspective: Scoring perspective ("demand" or "supply")
        user_max_price_mwh: User-specified maximum price per MWh
        user_ideal_mw: User-specified ideal capacity
        shared_component_scores: Pre-computed shared scores (for efficiency)
...
def _normalize_component_scores(component_scores: Dict[str, float]) -> Dict[str, float]:
    """Normalize component scores from 0-100 to 0-1."""
...
def _get_persona_tuning(persona: PersonaType) -> Dict[str, float]:
    """Get tuning parameters for a persona."""
...
def _compute_posterior_persona_weights(
    ...
    """Compute posterior persona weights based on evidence."""
...
def _calculate_target_alignment(
    ...
    """Calculate alignment with persona target components."""
...
def _logistic_transform(value: float, midpoint: float, steepness: float) -> float:
    """Apply logistic transformation to a value."""
...
def calculate_persona_weighted_score(
    ...
    """
    Calculate persona-weighted investment score for a project.
    ...
    Returns:
        Complete scoring result with ratings, components, and metadata
    """
...
def calculate_persona_topsis_score(
    ...
    """
    Calculate TOPSIS scores for multiple alternatives.

    Args:
        component_scores: List of component score dictionaries
        weights: Weight dictionary for each component

    Returns:
        TOPSIS analysis results with closeness coefficients
    """
...
def calculate_custom_weighted_score(
    ...
    """
    Calculate custom weighted score with user-defined weights.

    Args:
        project: Project dictionary
        proximity_scores: Dictionary containing infrastructure proximity data
        custom_weights: User-defined component weights

    Returns:
        Complete scoring result with custom weights applied
    """
...
def calculate_best_customer_match(
    ...
    """
    Find the best customer persona match for a project.
    ...
    Returns:
        Best customer match analysis with scores for all personas
    """
...
def filter_projects_by_persona_capacity(
    ...
    """
    Filter projects based on persona capacity requirements.
    ...
    Returns:
        Filtered list of projects matching persona capacity range
    """
    capacity_range = PERSONA_CAPACITY_RANGES[persona]
    filtered = []
