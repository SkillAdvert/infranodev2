...
# Tuning coefficients that control how component scores blend together.
#
# Higher ``beta`` and ``logistic_steepness`` values make the pipeline more
# sensitive to strong/weak components, while the non-zero
# ``product_weight``/``alignment_weight`` terms penalise inconsistent
# profiles instead of returning similar-looking averages.
PERSONA_SCORING_TUNING: Dict[str, Dict[str, float]] = {
    "hyperscaler": {
        "alpha": 1.0,
        "beta": 0.6,
        "evidence_floor": 0.05,
        "logistic_midpoint": 0.45,
        "logistic_steepness": 7,
        "evidence_shift": 0.35,
        "sum_weight": 0.5,
        "product_weight": 0.3,
